...
var Talent = {
	...
	Initialize : function() {
		...
		if( Class != "HUNTERPET" ){
		  ...
		  Talent.ptsDispTotal.innerHTML = Talent.MAX_PET_POINTS;
		}
	...
	InitPetTalentSettings : function() {
	  
		var div = document.createElement("div");
		
		Talent.pet_settings_container.appendChild(div);
		
		Talent.petSettings = new Array();
		
		for (sid in Data.settings) {
		  
		  var pretext = document.createElement("div");
		  div.appendChild(pretext);
		  pretext.innerHTML = Data.settings[sid]["pretext"];
		  pretext.style.padding = "8px";
		  
		  spell = Data.settings[sid]["icon"];
		  
		  var btn = document.createElement("div");
		  Talent.petSettings[sid] = btn;
		  ...
		  btn.style.margin = "20px 0px 20px 55px";
		  ...
		  $(btn).mousedown(Talent.OnPetSettingsButtonClick);
		
		
		  btn.onmouseover = function() {
			  Talent.ShowPetSettingsTooltip( btn );
		  }
		  ...
		  posttext.innerHTML = Data.settings[sid]["posttext"];
		  posttext.style.padding = "8px";
	...
	CreateTabSummary : function(tabId, data) {
		...
		if( Class != "HUNTERPET" ){
		  ...
		  div.desc.style.top = "35px";
		  div.appendChild(div.usable = document.createElement("ul"));
		  div.usable.className = "pet-usable-list";
		  ...
		  for (var i=0; i < usable.length; i++) {
		    ...
		    listIcon.src = data.usable[i].icon;
		    listIcon.className = "pet-usable-list-icon";
		    listIcon.height = 20;
		    listElement.appendChild( listIcon );
		    
		    var listName = document.createElement("span");
		    listName.className = "pet-usable-list-name";
		    listName.innerHTML = data.usable[i].name;
		    listElement.appendChild( listName );
		    
		    if( data.usable[i].type == 2 ){
		      var lkIcon = document.createElement("img");
		      lkIcon.src = "./img/wotlk.gif";
		      lkIcon.className = "pet-usable-list-icon-lk";
		      listElement.appendChild( lkIcon );
		    }else if( data.usable[i].type == 3 ){
		      var cataIcon = document.createElement("img");
		      cataIcon.src = "./img/cata.gif";
		      cataIcon.className = "pet-usable-list-icon-cata";
		      listElement.appendChild( cataIcon );
		    }else if( data.usable[i].type == 1 ){
		      var bcIcon = document.createElement("img");
		      bcIcon.src = "./img/bc.gif";
		      bcIcon.className = "pet-usable-list-icon-bc";
		      listElement.appendChild( bcIcon );
		    }
		    
		    if( data.usable[i].exotic ){
		      var exotic = document.createElement("span");
		      exotic.className = "pet-usable-list-exotic";
		      exotic.innerHTML = "exotic";
		      listElement.appendChild( exotic );
		    }
		    
		    div.usable.appendChild(listElement);
		  }
		}
	...
	IsTalentEnabled : function(btn) {
		...
		if( Class != "HUNTERPET" ){
		  ...
		  if (Talent.totalPoints == Talent.MAX_PET_POINTS)
			return btn.points > 0;
		}
		...
		if( Class != "HUNTERPET" ){
		  ...
		  return btn.tab.points >= Talent.POINTS_PER_ROW_PET * btn.spell.row;
		}
	},
	...
	GetIconTop : function(row, col) {
	  ...
	  if( Class != "HUNTERPET" ){
		  ...
		  return ((row + 1) * this.ICON_Y2_OFFSET + row * this.ICON_Y_SIZE);
		}
	...
	UpdatePointsDisplay : function() {
	  ...
	  if( Class != "HUNTERPET" ){
		...
		Talent.ptsDispTabs.innerHTML = Talent.tab[0].points + "/" + Talent.tab[1].points + "/" + Talent.tab[2].points + " &mdash; ";
		...
		$("#req-lvl").html(StaticData.PetPointsToLevel[Talent.totalPoints]);
	  }
	...
	OnPetSettingsButtonClick : function( e ) {	
		if (e.which == 3){ // rightclick
			Talent.DecPetSettingsRank( this );
		}else{
			Talent.IncPetSettingsRank( this );
		}
		
		return false;
	},
	
	IncPetSettingsRank : function( btn ){
	  ...
	  if( btn.spell.name == "Beast Mastery" ){
	    Talent.MAX_PET_POINTS = 21;
	    Talent.ptsDispTotal.innerHTML = Talent.MAX_PET_POINTS;
	  }
	  Talent.Update();
	  Talent.MakeLink();
	...
	DecPetSettingsRank : function( btn ){
	  if( btn.points == 0 ){
	    return;
	  }
	  
	  if( btn.spell.name == "Beast Mastery" ){
	    if( Talent.ptsDispCur.innerHTML > 17 ){
	      x = Talent.ptsDispCur.innerHTML -17;
	      var r=confirm("Unlearning [Beast Mastery] will decrease your total number of Pet Skill Points by 4.\n\nCurrently you have more than 17 talent points spent. ("+Talent.ptsDispCur.innerHTML+")\n\nConfirming this popup window will RESET all your POINTS.\n\nBy cancelling this popup window no changes will be made.\n\nIf you wish to unlearn [Beast Mastery], please MANUALLY unlearn "+x+" talent point(s) then try again." );
	      if (r==true){
		btn.points = 0;
		btn.rank.innerHTML = btn.points;
		btn.className = "talent-button-enabled";
		Talent.MAX_PET_POINTS = 17;
		Talent.ptsDispTotal.innerHTML = Talent.MAX_PET_POINTS;
		Talent.ResetAll();
	      }else{
	...
	IncRank : function(btn) {
		if( Class != "HUNTERPET" ){
		  if (Talent.totalPoints + 1 > Talent.MAX_POINTS)
		   ...
		   if (Talent.totalPoints + 1 > Talent.MAX_PET_POINTS)
			  return;
		}
	...
	DecRank : function(btn) {
		...
		while (typeof(btn.tab.rowPoints[r]) == "number") {
			...
			if (btn.tab.rowPoints[r] > 0 && points < r * Talent.POINTS_PER_ROW_PET)
				return; // would be invalid
		    }
			points += btn.tab.rowPoints[r];
			r++;
	...
	ShowTooltip : function(btn, init) {
		...
		if (init) {
			...
			if (typeof(tal.reqRank) == "number" && !Talent.RequirementsMet(btn, tal)) {
				...
				tt.reqTal.innerHTML = "Requires " + tal.reqRank + " points in [" + req.name + "]";
				$(tt.reqTal).show();
			}
			else {
				$(tt.reqTal).hide();
			...
			if( Class != "HUNTERPET" ){
			    if (btn.tab.points < Talent.POINTS_PER_ROW * tal.row) {
				    tt.reqPoints.innerHTML = "Requires " + Talent.POINTS_PER_ROW * tal.row + " points";
				    $(tt.reqPoints).show();
			    }
			    else {
				    $(tt.reqPoints).hide();
			    ...
			    if (btn.tab.points < Talent.POINTS_PER_ROW_PET * tal.row) {
				    tt.reqPoints.innerHTML = "Requires " + Talent.POINTS_PER_ROW_PET * tal.row + " points";
				    $(tt.reqPoints).show();
			    }
			    else {
				    $(tt.reqPoints).hide();
	...
	ShowPetSettingsTooltip : function( btn ) {
		
		var tt = Talent.tooltip;
		
		tt.spellName.innerHTML = btn.spell.name;
		tt.desc.innerHTML = btn.spell.desc;
		...
		Talent.PlaceTooltip(tt, btn, 25, 25);
	...
	ShowSpellTooltip : function(relTo, spell, tal, btn) {
		...
		if (!tal) {
			...
			if (typeof(tal.reqRank) == "number" && !Talent.RequirementsMet(btn, tal)) {
				var req = btn.tab.talents[tal.reqRow][tal.reqCol];
				tt.talReqTal.innerHTML = "Requires " + tal.reqRank + " points in [" + req.name + "]";
				$(tt.talReqTal).show();
			}
			else {
				$(tt.talReqTal).hide();
			...
			if( Class != "HUNTERPET" ){
			    if (btn.tab.points < Talent.POINTS_PER_ROW * tal.row) {
				    tt.talReqPts.innerHTML = "Requires " + Talent.POINTS_PER_ROW * tal.row + " points";
				    $(tt.talReqPts).show();
			    }
			    else {
				    $(tt.talReqPts).hide();
			    ...
			    if (btn.tab.points < Talent.POINTS_PER_ROW_PET * tal.row) {
				    tt.talReqPts.innerHTML = "Requires " + Talent.POINTS_PER_ROW_PET * tal.row + " points";
				    $(tt.talReqPts).show();
			    }
			    else {
				    $(tt.talReqPts).hide();
	...
	MakeLink : function() {
		...
		if( Class != "HUNTERPET" ){
		  ...
		  link += Talent.petSettings[0].points;	  
	...
	LoadLink : function(shortLink) {
		...
		for (i=0; i < 3; ++i) {
			...
			if( Class == "HUNTERPET" ){
			  if( Talent.totalPoints > Talent.MAX_PET_POINTS ){
			    alert( Talent.totalPoints +" / "+ Talent.MAX_PET_POINTS +" points. That's not right.\n\nResetting all talent points." );
			    Talent.ResetAll();
			  }
			}
		...
		if( Class != "HUNTERPET" ){
		  ...
		  if (link.charAt(0) == "!") {
		   ...
		   if (link.charAt(0) == "!"){
		     ...
		     if( link.substr(1) == "1" ){
			Talent.IncPetSettingsRank( Talent.petSettings[0] );
		     }else{
			Talent.DecPetSettingsRank( Talent.petSettings[0] );
		     }
		   }
		}
	},
	...
	SetEmptyGlyph : function(slot) {
		...
		slot.glyphName.innerHTML = "Empty";
	...
	MakeIngameMakro : function() {
		...
		prompt("Paste the following command to the chat.\n\nOnly works if the character has not yet learned talents.\n\nAfter pasting you will have to click learn talents.", makro);
